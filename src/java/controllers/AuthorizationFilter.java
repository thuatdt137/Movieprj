...
public class AuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (path.startsWith("/manageuser") || path.startsWith("/managegenre")
            ...
            if (!loggedIn) {
                ...
                return;
            } // Nếu đã đăng nhập nhưng không phải là admin, chuyển hướng về trang lỗi hoặc trang khác
            else if (!isAdmin(session)) {
                httpResponse.sendRedirect(httpRequest.getContextPath() + "/404");
                return;
            }
        }
        // Cho phép người dùng đã đăng nhập truy cập vào trang userprofile
    ...
    private boolean isAdmin(HttpSession session) {
        // Kiểm tra quyền của người dùng từ session (0 là admin)
